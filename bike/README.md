...
###  Förklaring
Bike fungerar som en express server och är tänkt att kommunicera cyklars/scootrars rörelse.
Uppdateringar skickas med hjälp av sockets till backend som lagrar ändringar i databasen. 
...
Simuleringar fungerar med hjälp av interval som går baserat på en "tickspeed" vilket då motsvarar hur fort en scooter byter koordinat längts sin definierade rutt.
Tickspeed kan justeras dynamiskt mellan simuleringar. Det samma gäller med sändningsfrekvensen, alltså hur ofta en uppdatering av en cykels data skickas och uppdateras.
...
### Användning
Enklast kommer man igång genom att köra ```docker-compose run --rm bike ```, vilket kommer köra igång osrm och backend också.
Om allt går väl så skapas en koppling mot backend och systemet är redo att skicka uppdateringar till databasen.
För att se vad som händer på backend:ens sida kan det vara nyttigt att koppla upp via ```docker-compose logs -f backend``` i en egen ruta.
...
```broadcastEnable``` slår på/av kommunikation med backend. Om kommunikationen är av så är det en helt lokal körning och den påverkar inte datan i databasen.
```broadcastRate``` ställer in hur ofta data skickas till backend.
```tickrate``` ställer in hur ofta scootrar byter sin position. Lite högre värde är mer trovärdigt i hänsyn till hur fort en scooter rör sig i verkligheten. 
```tickLimit``` ställer in hur många steg som tillåts i simuleringen. Om alla rutter inte är färdiga, så kommer simuleringen försöka göra klart de rutter som redan är igång.
